...
class CLIPVisionCfg:
    ...
    subln: bool = False
    use_alpha_channel: bool = False  # 新增alpha通道配置
    # TODO 给LLM2CLIP添加 alpha通道
    use_alpha_channel: bool = True  # 新增alpha通道配置
...
def _build_vision_tower(
    ...
    if vision_cfg.eva_model_name:
        ...
        visual = EVAVisionTransformer(
            ...
            # TODO  给LLM2CLIP添加alpha通道
            use_alpha_channel=vision_cfg.use_alpha_channel  # 新增参数
        )
...
class CustomCLIP(nn.Module):
    def __init__(
            ...
            itm_task: bool = False,
            # TODO 是否提前抽取特征，降低占用
            pre_extract_feature: bool = False,
    ):
        super().__init__()
        self.pre_extract_feature = pre_extract_feature
        self.visual = _build_vision_tower(embed_dim, vision_cfg, quick_gelu, cast_dtype)
        self.text = _build_text_tower(embed_dim, text_cfg, quick_gelu, cast_dtype)
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        

        # TODO 将LLM2CLIP的LLM encoder给封装进这个类里面去
        if not self.pre_extract_feature:
            ...
            print("We have pre-extract features. Not load text tower")
    ...
    def encode_image(self, image, alpha=None,normalize: bool = False, forward_head=False):
        
        # TODO add alpha channel support
        if forward_head:
            features = self.visual.head(image, alpha)
        else:
            features = self.visual(image, alpha)
        return F.normalize(features, dim=-1) if normalize else features

    def encode_text(self, text, normalize: bool = False):
        if not self.pre_extract_feature:
            # TODO 没有提前抽取特征
            # TODO 先用l2v对dense caption进行编码，在送进text adapter中去
            text_features = self.l2v.encode(text, convert_to_tensor=True).to(next(self.text.parameters()).device)
            features = self.text(text_features)
        else:
            # TODO 已经提前抽取好了特征
    ...
    def forward(self, image, alpha, text):
        # TODO 添加alpha channel支持
        image_features = self.encode_image(image, alpha,normalize=True, forward_head=False)
        text_features = self.encode_text(text, normalize=True)
        return image_features, text_features, self.logit_scale.exp()
...
def trace_model(model, batch_size=256, device=torch.device('cpu')):
    """
    将CLIP模型转换为jit追踪模型，这样可以提高模型在推理阶段的性能
    ...
    # 使用torch.jit.trace_model对模型进行追踪
    # 追踪三个主要方法：forward、encode_text和encode_image
