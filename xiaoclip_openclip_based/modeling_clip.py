...
class CLIPVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor, alpha_values: torch.FloatTensor) -> torch.Tensor:
        # TODO 添加alpha 通道的输入
        batch_size = pixel_values.shape[0]
        target_dtype = self.patch_embedding.weight.dtype

        patch_embeds = self.patch_embedding(pixel_values.to(dtype=target_dtype))  # shape = [*, width, grid, grid]
        alpha_embeds = self.alpha_patch_embedding(alpha_values.to(dtype=target_dtype))  # shape = [*, width, grid, grid]
        patch_embeds = patch_embeds.flatten(2).transpose(1, 2)
        alpha_embeds = alpha_embeds.flatten(2).transpose(1, 2)
        
        # TODO 加上alpha通道的embeds
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward_features_new(
        ...
        r"""
        Args:
            pixel_values (torch.FloatTensor, optional): Pixel values.
            alpha_values (torch.FloatTensor, optional): Alpha values.
            output_attentions (bool, optional): Whether or not to return the attentions tensors of all attention layers.
            output_hidden_states (bool, optional): Whether or not to return the hidden states of all layers.
            return_dict (bool, optional): Whether or not to return a :class:`~transformers.modeling_outputs.BaseModelOutputWithPooling` instead of a plain tuple.

        Returns:
            BaseModelOutputWithPooling or tuple: The model output. If `return_dict=True`, returns :class:`~transformers.modeling_outputs.BaseModelOutputWithPooling`.
        """
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        ...
        last_hidden_state = encoder_outputs[0]
        output_features = self.post_layernorm(last_hidden_state)
        output_features = output_features[:, 1:, :]

        B, N, C = output_features.shape
        H = W = int(N ** 0.5)  # 假设是正方形图像
        patch_feature_map = output_features.transpose(1, 2).reshape(B, C, H, W)  # [B, C, H, W]
        return patch_feature_map
...
class LLM2CLIPModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        # TODO 是否要抽取特征，一定要注意修改
        self.pre_extract_feature = False

        if not self.pre_extract_feature:
            # TODO 没有提前抽取特征
            print("如果没有提前抽取特征")
            ...
            self.l2v = LLM2Vec(llm_model, tokenizer, pooling_mode="mean", max_length=512, doc_max_length=512)
            # print("device of self.text :",  next(self.text.parameters()).device)
            # self.text.eval()            
    ...
    def get_image_features(
        self,
        pixel_values: Optional[torch.FloatTensor] = None,
        alpha_values: Optional[torch.FloatTensor] = None, # 添加了alpha通道输入
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> torch.FloatTensor:
    ...
    # TODO 为了适配训练，自定义的接口
    def encode_image(self, pixel_values: torch.FloatTensor, alpha_values: torch.FloatTensor, normalize: bool = False) -> torch.FloatTensor:
        image_features = self.get_image_features(pixel_values, alpha_values)
        if normalize:
            image_features = image_features / image_features.norm(p=2, dim=-1, keepdim=True)
        return image_features

    def encode_text(self, text,normalize: bool = False) -> torch.FloatTensor:
        if not self.pre_extract_feature:
            # TODO 没有提前抽取特征
            print("如果没有提前抽取特征")
            text_features = self.l2v.encode(text, convert_to_tensor=True).to(next(self.text_adapter.parameters()).device)
            features = self.get_text_features(text_features)
        else:
            # 如果提前抽好了特征
