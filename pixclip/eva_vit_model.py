...
class EVAVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 use_alpha_channel=True  #添加alpha channel
                ):
        super().__init__()
        ...
        # TODO 添加Alpha通道的patch embedding
        # 添加Alpha通道的patch embedding
        if use_alpha_channel:
            self.alpha_patch_embed = PatchEmbed(
    ...
    def forward_features(self, x, alpha=None, return_all_features=False):
        ...
        if alpha is not None:
            alpha_x = self.alpha_patch_embed(alpha)
            x = x + alpha_x            
    ...
    def forward_features_new(self, x, alpha=None, return_all_features=False):
        ...
        if not return_all_features:
            ...
            if self.fc_norm is not None:
                ...
                return x[:, 0]
                  ...
                  #print("2")
                  # 返回 patch tokens 的 feature map（去掉 cls token）
                  x = self.norm(x)
                  patch_tokens = x[:, 1:]  # 去掉 cls token

                  B, N, C = patch_tokens.shape
                  H = W = int(N ** 0.5)  # 假设是正方形图像
                  patch_feature_map = patch_tokens.transpose(1, 2).reshape(B, C, H, W)  # [B, C, H, W]
                  return patch_feature_map        


    def forward(self, x, alpha=None, return_all_features=False):
        # TODO 添加alpha通道参数
